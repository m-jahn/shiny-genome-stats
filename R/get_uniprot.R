get_uniprot <- function(query, reviewed = NULL) {
  ...
  uniprot_url <- paste0(
    ...
    "%2Creviewed%2Cid%2Cprotein_name%2Cgene_names%2Corganism_name%2Cxref_refseq_full",
    "%2Clength%2Cgene_oln%2Ccc_pathway%2Cgo_id%2Cgo_p%2Ccc_subcellular_location",
    "&format=tsv&query=%28%28", query_type, "%3A", query, "%29%29", reviewed
  )
  ...
  if (nrow(df_uniprot) > 10000) {
    stop(
      paste(
        ...
        "Please make your strain name or taxonomy ID more specific.",
        sep = "\n"
      )
    )
  }
